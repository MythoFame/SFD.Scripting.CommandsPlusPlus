...
public partial class GameScript : GameScriptInterfaceExtended
{
    public sealed partial class FunModule
    {
        private static void Lightning(UserMessageCallbackArgs args)
        {
            ...
            if (tokens.Length != 1)
            {
                Game.ShowChatMessage("Usage: /lightning <player>", Color.Red, args.User.UserIdentifier);
                return;
            }
            ...
            Game.ShowChatMessage($"Struck {affected} player(s) with lightning.", Color.Green, args.User.UserIdentifier);
        }
    }
}
